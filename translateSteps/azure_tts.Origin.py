...
def parse_time_ms(s: str) -> int:
    """
    Parse a timestamp string in the format hh:mm:ss.ms and return milliseconds.
    """
    hh, mm, rest = s.split(":")
    ss, ms = rest.split(".")
...
def read_items(path: str):
    """
    Read a file with timestamped lines and return a list of items with start, end, and text.
    Each item is a dict: {"start": ms, "end": ms, "text": str}
    """
    items = []
...
# --- Build SSML with custom speaking rate ---
def build_ssml(text: str, xml_lang: str, voice_name: str, rate_val: float) -> str:
    ...
    rate_val = 1.0 is normal, <1 is slower, >1 is faster.
    """
    # Map the numeric rate to Azure's SSML rate strings
    if rate_val > 1.4:   # Very fast
        rate_str = "x-fast"
    elif rate_val > 1.08:
        rate_str = "fast"
    elif rate_val > 0.85:
...
def synthesize_ssml(text: str, out_path: str, voice: str, xml_lang: str, seg_duration: int):
    """
    Synthesize speech for the given text and save it to out_path.
    The speech rate is automatically adjusted to fit the segment duration.
    If text is empty, generate silence for the segment duration.
    """
    if not text.strip():
        # If the text is empty, export silence for the segment
        silent = AudioSegment.silent(duration=seg_duration)
        silent.export(out_path, format="wav")
        return
    chars_per_sec = 13.0  # Average: 13 characters per second for "normal" speech (adjust as needed)
    est_normal_ms = len(text) / chars_per_sec * 1000
    rate_val = est_normal_ms / seg_duration  # How fast/slow to speak to fit the segment
    # Clamp the rate to reasonable values (do not allow x2 or 0.5 speed)
    rate_val = min(max(rate_val, 0.7), 1.5)
...
def render_by_timestamps(timed_path: str, out_wav: str, lang_code: str):
    ...
    try:
        ...
        for i, item in enumerate(items):
            ...
            synthesize_ssml(item["text"], seg_path, voice, xml_lang, seg_duration)
