...
def transcribe_latest_audio():
    ...
    files = [f for f in os.listdir(audio_dir) if f.lower().endswith((
        ".wav", ".mp3", ".m4a", ".ogg", ".flac", ".mp4", ".aac", ".wma", ".alac", ".opus"))]
    if not files:
    ...
    # 4b. Save plain text (without timestamps)
    plain_text_output = os.path.join(text_dir, f"{base_name}_plain.txt")
    plain_lines = [segment["text"] for segment in result["segments"]]
    with open(plain_text_output, "w", encoding="utf-8") as f:
        f.write(" ".join(plain_lines))  # One long paragraph. Use "\n".join(...) if you want line-by-line.
    print(f"Plain transcription saved to: {plain_text_output}")

    # 5. Move the audio to audioDone
    shutil.move(audio_path, os.path.join(done_dir, latest_file))
    print(f"Audio moved to: {done_dir}")

    return {"result": "success", "txt_file": output_txt, "plain_txt_file": plain_text_output}
